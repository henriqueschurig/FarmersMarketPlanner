...
if uploaded_file is not None:
    ...
    cost_info = filtered_calendar_df[["Name", "Day of the Week", "Start", "End", "Total Weeks", "Weekly Fee", "Total Cost"]]
    ...
    try:
        ...
        avg_hour_cost = 0
    if total_cost != 0:
        st.write(f"The Total Cost is \${total_cost:,.2f} for {num} Farmers' Market for a 10x10 squared foot booth. Each Farmers' Market costs on average \${avg_fm_cost:,.2f}. Each Hour costs on average \${avg_hour_cost:,.2f}")
    else:
